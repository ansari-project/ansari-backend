...
def add_message(url, token, thread_id, role, content):
    ...
    response = requests.post(
        url + "/api/v2/threads/" + str(thread_id),
...
def set_thread_name(url, token, thread_id, name):
    ...
    response = requests.post(
        url + "/api/v2/threads/" + str(thread_id) + "/name",
        headers={"Authorization": "Bearer " + token, "x-mobile-ansari": "ANSARI"},
        json={"name": name},
    )
    print(f"Response is {response}")
    return response.json()
...
def refresh_token(url, token):
    ...
    response = requests.get(
        url + "/api/v2/users/refresh_token",
        headers={
            "Authorization": "Bearer " + token,
            "x-mobile-ansari": "ANSARI",
        },
...
register(
    default_url, email_address, str(random_pass), f"Waleed {random_number}", "Kadous"
)
