...
class AnsariClaude(Ansari):
    ...
    def process_one_round(self) -> Generator[str, None, None]:
        ...
        while not response:
            try:
                ...
                # Log specific details for tool_use errors
                if "tool_use" in str(e) and "tool_result" in str(e):
                    logger.error("Claude API error with tool_use/tool_result relationship")
                    
                    # Add additional logging to help troubleshoot
                    tool_use_count = sum(
                        sum(1 for block in msg.get("content", []) 
                    ...
                    logger.error(f"Total tool_use blocks: {tool_use_count}, Total tool_result blocks: {tool_result_count}")
                    
                    if failures == 1:  # Only try this fix on the first failure
                        logger.warning("Attempting aggressive fix of message history tool_use/tool_result relationship")
                        # Apply comprehensive fix and update the message history
                        self._fix_tool_use_result_relationship()
                        # Update the limited history with fixed version
                        limited_history = self.limit_documents_in_message_history(max_documents=100)
                        params["messages"] = limited_history
    ...
    def process_message_history(self, use_tool=True, stream=True):
        ...
        tool_use_ids = set()
        tool_use_to_result_map = {}

        # First pass: collect all tool_use IDs and ensure each has a corresponding tool_result
            ...
            if msg.get("role") == "assistant" and isinstance(msg.get("content"), list):
                for block in msg["content"]:
                    if isinstance(block, dict) and block.get("type") == "tool_use" and "id" in block:
                        ...
                        tool_use_to_result_map[tool_id] = False  # Mark as not having a result yet
                        logger.debug(f"Found tool_use block with ID: {tool_id}")
            
            # Check for tool_result blocks and mark the corresponding tool_use as having a result
            elif msg.get("role") == "user" and isinstance(msg.get("content"), list):
                for block in msg["content"]:
                    if isinstance(block, dict) and block.get("type") == "tool_result" and "tool_use_id" in block:
                        tool_id = block["tool_use_id"]
                        if tool_id in tool_use_to_result_map:
                            tool_use_to_result_map[tool_id] = True
                            logger.debug(f"Found tool_result block for tool_use ID: {tool_id}")

        logger.debug(f"Found tool_use_ids: {tool_use_ids}")
        logger.debug(f"Tool use to result map: {tool_use_to_result_map}")

        # Find any tool_use blocks that don't have corresponding tool_result blocks
        missing_tool_results = [tool_id for tool_id, has_result in tool_use_to_result_map.items() if not has_result]
        if missing_tool_results:
            logger.warning(f"Found tool_use blocks without corresponding tool_result blocks: {missing_tool_results}")
            
            # Fix message history by adding missing tool_result blocks
            for tool_id in missing_tool_results:
                logger.info(f"Adding fallback tool_result block for tool_use ID: {tool_id}")
                ...
                # Add a fallback tool result message
                    ...
                    "content": [
                        ...
                        },
                        {
                            ...
                            "source": {
                                "type": "text",
                                "media_type": "text/plain",
                                "data": f"No content found for tool use ID: {tool_id}"
                            },
                ...
                # Insert the fallback message directly after the corresponding tool_use message
                for i, msg in enumerate(self.message_history):
                    if msg.get("role") == "assistant" and isinstance(msg.get("content"), list):
                        for block in msg["content"]:
                            if isinstance(block, dict) and block.get("type") == "tool_use" and block.get("id") == tool_id:
                                # Insert after this message
                                self.message_history.insert(i + 1, fallback_result_message)
                                logger.info(f"Inserted fallback tool_result block after position {i}")
                                break
        ...
        for i in range(len(self.message_history)):
            ...
            # User messages with tool_result need to have matching tool_use blocks and document blocks
            elif msg.get("role") == "user" and isinstance(msg.get("content"), list):
                fixed_content = []
                has_invalid_tool_result = False
                has_tool_result = False
                needs_document_block = False
                ...
                for block in msg["content"]:
                    ...
                    if is_tool_result:
                        ...
                        needs_document_block = True

                    # Document blocks come after tool_result blocks
                    elif isinstance(block, dict) and block.get("type") == "document" and needs_document_block:
                        needs_document_block = False  # We found at least one document block
                    
                    # Keep this block
                    fixed_content.append(block)

                # If we have a tool_result but no document block, add a fallback document block
                if has_tool_result and needs_document_block:
                    logger.warning(f"Tool result message at position {i} is missing document blocks - adding fallback document")
                    tool_id = None
                    tool_name = "unknown"
                    
                    # Find the tool_use_id from the tool_result block
                    for block in fixed_content:
                        if isinstance(block, dict) and block.get("type") == "tool_result" and "tool_use_id" in block:
                            tool_id = block["tool_use_id"]
                            
                            # Try to get the tool name from the corresponding tool_use block
                    ...
                    fixed_content.append(fallback_doc)
