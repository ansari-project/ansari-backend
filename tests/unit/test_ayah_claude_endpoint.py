...
class TestAyahClaudeEndpoint:
    ...
    def test_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/v2/ayah-claude", json={"surah": 1, "ayah": 1, "question": "What is the meaning?", "apikey": "wrong-key"}
        )
    ...
    def test_authentication_required(self, client, mock_settings):
        ...
        response = client.post(
            "/api/v2/ayah-claude", json={"surah": 1, "ayah": 1, "question": "What is the meaning?", "apikey": "wrong-api-key"}
        )
    ...
    def test_successful_request_with_valid_key(self, client, mock_settings, mock_db, mock_ansari_claude):
        ...
        with patch("builtins.open", mock_open(read_data="Test system prompt")):
            ...
            response = client.post(
                "/api/v2/ayah-claude",
                json={"surah": 1, "ayah": 1, "question": "What is the meaning?", "apikey": "test-api-key", "use_cache": True},
            )
    ...
    def test_cache_disabled(self, client, mock_settings, mock_db, mock_ansari_claude):
        ...
        with patch("builtins.open", mock_open(read_data="Test system prompt")):
            ...
            response = client.post(
                "/api/v2/ayah-claude",
                json={"surah": 2, "ayah": 255, "question": "Explain this verse", "apikey": "test-api-key", "use_cache": False},
            )
    ...
    def test_database_storage(self, client, mock_settings, mock_db, mock_ansari_claude):
        ...
        with patch("builtins.open", mock_open(read_data="Test system prompt")):
            ...
            response = client.post(
                "/api/v2/ayah-claude",
                json={"surah": 4, "ayah": 34, "question": "Explain the context", "apikey": "test-api-key", "use_cache": True},
            )

            assert response.status_code == 200
            # Verify that store_quran_answer was called
            mock_db.store_quran_answer.assert_called_once_with(
                4, 34, "Explain the context", "This is a test response about the ayah with citations."
            )
    ...
    def test_ayah_specific_system_prompt(self, client, mock_settings, mock_db, mock_ansari_claude):
        ...
        with patch("builtins.open", mock_open(read_data=system_prompt_content)):
            ...
            response = client.post(
                "/api/v2/ayah-claude",
                json={"surah": 5, "ayah": 3, "question": "What is the significance?", "apikey": "test-api-key"},
            )
    ...
    def test_ayah_id_calculation(self, client, mock_settings, mock_db, mock_ansari_claude):
        ...
        with patch("builtins.open", mock_open(read_data="Test system prompt")):
            ...
            response = client.post(
                "/api/v2/ayah-claude",
                json={"surah": 2, "ayah": 255, "question": "Explain Ayat al-Kursi", "apikey": "test-api-key"},
            )
    ...
    def test_error_handling(self, client, mock_settings, mock_db):
        ...
        with patch("src.ansari.app.main_api.AnsariClaude") as mock_claude:
            ...
            with patch("builtins.open", mock_open(read_data="Test system prompt")):
                response = client.post(
                    "/api/v2/ayah-claude", json={"surah": 1, "ayah": 1, "question": "Test question", "apikey": "test-api-key"}
                )
    ...
    def test_request_validation(self, client, mock_settings):
        ...
        # Invalid data types
        response = client.post(
            "/api/v2/ayah-claude", json={"surah": "not-a-number", "ayah": 1, "question": "Test", "apikey": "test-key"}
        )
